...
class TcpClient {
 ...
 protected:
  ...
  struct addrinfo clientSocketSettings;
 ...
 protected:
  ...
  /// return OK_SOCKET if all success 
  int fetchPossibleAddresses(const char* server, const char* port);
  /// Get a socket for the first available address that works
  /// return OK_SOCKET if all success 
  int openSocketWithServer(const char* server, const char* port);
};
